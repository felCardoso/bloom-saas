  ...
  return (
    <Card padding="none">
      <div className="px-5 py-4 border-b border-neutral-100 dark:border-neutral-800 flex items-center justify-between">
        <h3 className="text-base font-semibold text-neutral-800 dark:text-neutral-100">Pedidos Recentes</h3>
        <Link
          href="/pedidos"
      ...
      <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
        {orders.map((order) => {
          ...
          return (
            <div key={order.id} className="px-4 lg:px-5 py-3.5 flex items-center gap-3 hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
              <Avatar name={order.client_name} size="sm" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-neutral-800 dark:text-neutral-100 truncate">{order.client_name}</p>
                <p className="text-xs text-neutral-400 dark:text-neutral-500">
                  {order.items.length} {order.items.length === 1 ? "item" : "itens"} · {formatDate(order.created_at)}
                </p>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-100">{formatCurrency(order.total)}</p>
